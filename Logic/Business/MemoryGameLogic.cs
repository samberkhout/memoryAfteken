...
namespace memoryAfteken.Business
{
    public class MemoryGameLogic
    {
        ...
        public void FlipCard(int cardIndex)
        {
            ...
            // Increment the number of attempts each time a card is flipped
            _numberOfAttempts++;
        ...
        public async Task EndGameAsync(string playerName)
        {
            ...
            var timeTaken = (_endTime - _startTime).TotalSeconds;

            // Calculate final score based on the formula
            double finalScore = ((double)(_cards.Count * _cards.Count) / (timeTaken * _numberOfAttempts)) * 1000;
            ...
            try
            {
                ...
                if (highScores.Count < 10 || finalScore > highScores[highScores.Count - 1].Score)
                {
        ...
        {
            get { return _score; }
            set { _score = value; } // Add a setter here
        }
